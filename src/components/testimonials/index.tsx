                ...
                {[1, 2, 3].map((colNum, key) => {
                    return <div key={key} className="col-md-4">
                        {parsedText(props).text.map((test, index) => {
                            return test.index % 3 === (colNum - 1) ? <span key={index}>
                                <div className="test-box" data-sal="zoom-in"
                                    data-sal-easing="ease"> <p className="title">{test.title}</p> <p className="test-details">{test.content}</p>
                                    <div><span className="test-from">{test.from} ({test.fromCompany})</span></div>
                                </div>
                            </span> : null
                        })}
                    </div>
